...
проверил - кратинка открывается:

![Alt text](image-11.png)

А если просто сайт открыть - открывается сайты через haproxy:

![Alt text](image-12.png)

Конфиг HAPROxy не менялся - использовал балансировку на 4м уровне:
...
А тут что-то не понял как настроить. Пробовал двойку подписать вконце - не полчалось.
Начал делать конфиг:
...
#    use_backend web_servers if ACL_example.com
    ...
    use_backend web_servers if ACL_example2.local
...
backend web_servers    # секция бэкенд
        ...
        option httpchk
        http-check send meth GET uri /index.html
        server s1 127.0.0.1:8888 inter 3s
        server s2 127.0.0.1:9999 inter 3s
...
#        option httpchk
#        http-check send meth GET uri /index.html
#        server s3 127.0.0.1:7777 inter 3s
#        server s3 127.0.0.1:6666 inter 3s
```

Но, не понял как дальше?
